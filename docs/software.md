...
* Функція `register_user` відповідає за реєстрацію нового користувача. Вона перевіряє, чи не існує вже користувач з вказаною електронною поштою, і, якщо ні, додає нового користувача до таблиці Users.
* Функція `authenticate_user` відповідає за авторизацію користувача. Вона перевіряє, чи збігаються введені електронна пошта та пароль з даними в базі даних.
* Функція `update_user_role` відповідає за зміну прав доступу користувача. Вона перевіряє, чи має адміністратор достатні права для зміни прав доступу іншого користувача.
* Функція `delete_user` відповідає за видалення користувача з бази даних за електронною поштою.
